...
int8_threshold = 4.0
    ...
    # LLM Model choosen
model_creator = "Qwen"
folder_name = "qwen_3.0_32B"
model_id = "Qwen3-32B"
model_name = f"{model_creator}/{model_id}"
...
exp_big_str = f"<PROJECT_ROOT>/questions_random_w_exp/combined_redundant_{diff_level}.py"
...
embedding_store = f'<PROJECT_ROOT>/formal_exp_vector_db/semantic_rag{rag_setting}_{chunk_metric}{percentile}_level_{diff_level}'
...
if not os.path.exists(embedding_store):
    ...
    # Print sample chunks
    print(texts_splits[10])
    print(texts_splits[50])
    print(texts_splits[60])
...
You are an anesthesiologist having an exam. Please answer this question according to the context information.
Context: {context}
...
for ques_num in range(1,4,1):
    ...
    log_name = f"{test_type}_{max_new_tokens}_{int8_threshold}_{model_id}_log_ques{ques_num}_level_{diff_level}"
    result_log = f'<PROJECT_ROOT>/data_result_formal_1/{folder_name}/{test_type}/{log_name}.txt'
    asa_data_path = f'<PROJECT_ROOT>/questions_random_w_exp/ques_100_set_{ques_num}_w_exp.csv'
    answer_csv_path = f'<PROJECT_ROOT>/data_result_formal_1/{folder_name}/{test_type}/{log_name}.csv'
