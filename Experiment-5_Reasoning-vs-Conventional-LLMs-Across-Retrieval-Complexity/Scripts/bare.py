...
generate_prompt = PromptTemplate(
    template="""<|begin_of_text|><|start_header_id|>system<|end_header_id|> You are an anesthesiologist having an exam. Please answer this question according to your trained expertise.
...
def generate(user_prompt, gen_chain):
    print("---ANSWERING....---")
    model_input = user_prompt + Answer_format
    print(f"INPUT PROMPT TO MODEL: {model_input}\n")
    generation = gen_chain.invoke({"question": model_input})
...
for ques_num in range(1,4,1):
    ...
    log_name = f"{test_type}_{max_new_tokens}_{int8_threshold}_log_ques{ques_num}"
    result_log = f'<PROJECT_ROOT>/{test_type}/{log_name}.txt'
    asa_data_path = f'<PROJECT_ROOT>/ques_100_set_{ques_num}_w_exp.csv'
    answer_csv_path = f'<PROJECT_ROOT>/data_result_formal_1/{test_type}/{log_name}.csv'
...
Choose only one answer (A), (B), (C), (D) or (E) and give explaination in step by step.\n"""
        ...
        iter_answer = generate(prompt, rag_generate_chain)
